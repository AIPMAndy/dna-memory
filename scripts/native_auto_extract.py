...
_ASSISTANT_RULES = (
    ("fact", re.compile(r"(?:已验证[：:]?|验证结果[：:]|测试通过[：:]?|已修复[：:]?|根因[：:]|结论[：:]|verified[：:]?|tests? pass(?:ed)?[：:]?|root cause[：:])", re.IGNORECASE)),
    ("workflow", re.compile(r"(?:流程是|步骤是|应当先|应该先|the workflow|steps are)")),
)
_TRANSIENT = re.compile(r"(?:现在|今天|明天|今晚|待会|下午|上午|if I am|later today)", re.IGNORECASE)
...
_META_MEMORY = re.compile(r"(?:人工审查|自动提取|误采集|记忆检索|候选(?:里|中|项)|DNA_MEMORY_PROPOSAL)", re.IGNORECASE)
...
def _record_message(record):
    ...
    body = record
    if role not in {"user", "assistant"} and record.get("type") in {"user", "assistant"}:
        role = record["type"]
    if role not in {"user", "assistant"} and record.get("type") == "response_item":
...
def _sentence(text):
    ...
    return next((part.strip() for part in parts if part.strip()), line)
...
def extract_automatic_proposals(messages, max_proposals=DEFAULT_MAX_PROPOSALS):
    ...
    for message in messages:
        text = _sentence(message.get("content", ""))[:800].strip()
        if not text:
            continue
        if (_MEMORY_ECHO.search(text) or _META_MEMORY.search(text)
            ...
            continue
        if message.get("role") == "user" and _TRANSIENT.search(text):
            continue
        if message.get("role") == "assistant" and _VAGUE_ASSISTANT.match(text):
            continue
        rules = _USER_RULES if message.get("role") == "user" else _ASSISTANT_RULES
        for memory_type, pattern in rules:
            ...
            proposals.append({
                ...
                "importance": 0.8 if memory_type in {"preference", "decision"} else 0.7,
            })
            break
...
def _session_id(path, messages):
    try:
        ...
        data = json.loads(raw.decode("utf-8")) if Path(path).suffix == ".json" and size <= len(raw) else {}
        if isinstance(data, dict):
            return data.get("session_id") or data.get("sessionId") or data.get("cliSessionId")
...
def import_native_file(path, queue, client, max_bytes=DEFAULT_MAX_BYTES):
    ...
    if checkpoint and checkpoint[2] == digest:
        return {"processed": False, "enqueued": 0, "proposals": 0}
    ...
    return {"processed": True, "enqueued": enqueued, "proposals": len(proposals)}
